...
\newcommand{\github}{\url{https://github.com/Snuggan/pax2}}
\newcommand{\dockerhub}{\url{https://hub.docker.com/r/axensten/slu}}
...
The Remote Sensing section of the Department of Forest Resources at the Swedish Agricultural University (\textsc{slu}) has developed a framework and tools to estimate forest variables wall-to-wall for Sweden\footnote{\github}. The data are from the {\als} produced by the Swedish Land Survey and field data are from the Swedish National Field Inventory, produced at \textsc{slu}. The framework handles all parts of the process ensuring that all files that need to be processed are processed (in parallel) through all parts of the process. The process has the following principal parts:
\begin{enumerate}
	...
	\item modelling and prediction, and
	\item post-processing: mosaics and evaluation.
\end{enumerate}

\vspace{0.5em }{\noindent}The processing unit (\pu) is the area covered by an individual point data file. These files (and their offsprings) may be processed in parallel, as there are no algorithmic dependencies between them. 

\section{Implementation}

\subsection{Framework}

To coordinate the processing of all files through the processing steps a {\make} script is used. If a file is added or changed upstream the dependency chain, {\make} ensures that all dependent files downstream are created or updated. This greatly simplifies updating the data set by only processing affected files. A {\make} script defines these dependencies and what tools to execute to produce or update the downstream files. It programmatically describes the complete process. 

Once everything is set up properly, all you need to do to process all files (often in the hundreds of thousands) through all parts of the process is to execute one command in the command prompt:
\\[0.25em]\mbox{}\hspace{5em}{\texttt{make -j12 -k}}
\\[0.25em]The argument \texttt{-j12} tells {\make} to use up to 12 threads – use as many as your machine can muster, although a very large number of threads may saturate the file storage bus. Parameter \texttt{-k} tells {\make} not to stop if it encounters an error, so if you have a corrupt file {\make} will continue processing files that are independent of it. 

\subsection{Tools}

The two point data processing tools used in the first parts of the process are based on {\pdal}, a general tool for point cloud processing. Its modules may be combined and executed sequentially in one command, so that the point cloud file is read once, run through the modules as specified, and finally the resulting file is saved. Some of the modules used are included with {\pdal}, but other had to be developed. 
...
\begin{enumerate}
	...
	\item \textbf{Altitude and point class filtering} (proprietary module \texttt{filters.slu\_lm}). All points with $z$-values below {\zmin} or above {\zmax} are removed. For the remaining points, all $z$-values below zero are set to zero. Only points with any of the these classes are retained:%
...
\paxbox{Filtered point data file + plot data file $\rightarrow$ metric raster files + metrics plot file.}
A {\pdal} tool with the following modules that are executed sequentially:
\begin{enumerate}
...
\subsection{{\docker}}
All processing tools, the framework, and the processing environment are included in an available Ubuntu-based {\docker} image\footnote{\dockerhub}. So if your system has {\docker}, you can install the image and run the tools individually or use the framework {\make} script. Some of the proprietary {\pdal} modules might be of use individually. Execute \verb|pdal --options filters.<name>| to get detailed information on their use. 
\begin{itemize}
...
[Authored by Jörgen and/or Mats?]
