#include <pax/pdal/modules/pdal_plugin_filter_raster_metrics_old.hpp>
...
static pdal::PluginInfo const s_info {
		"filters.raster_metrics_old",
		"Create rasters with specified metrics (statistics). "
		"Execute 'pdal --options filters.raster_metrics_old' for a list of available parameters and metrics. ",
		"This filter is deprecated."
};

CREATE_SHARED_STAGE( pax::raster_metrics_old, s_info )
std::string pax::raster_metrics_old::getName()	const	{	return s_info.name;		}
...
void pax::raster_metrics_old::addArgs( pdal::ProgramArgs & args ) {
...
void pax::raster_metrics_old::addDimensions( pdal::PointLayoutPtr layout_ ) {
...
pax::raster_metrics_old::~raster_metrics_old() {
}



pdal::PointViewSet pax::raster_metrics_old::run( pdal::PointViewPtr view_ ) {
