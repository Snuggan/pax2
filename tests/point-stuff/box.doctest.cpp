...
namespace pax {
	...
	DOCTEST_TEST_CASE( "Box_indexer object" ) {
		...
		{	// 2d Box_indexer
			...
			DOCTEST_FAST_CHECK_EQ( boxi.offsets(), 			Index2d{ 1u, 4u } );
			DOCTEST_FAST_CHECK_EQ( boxi.elements(), 		8 );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 2. } ), 	0u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 2. } ), 	3u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 6. } ), 	4u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 6. } ), 	7u );
			DOCTEST_FAST_CHECK_EQ( boxi.point( index( 0u, 0u ) ), 	Point{ 0., 2. } );
			DOCTEST_FAST_CHECK_EQ( boxi.point( index( 3u, 0u ) ), 	Point{ 6., 2. } );
			DOCTEST_FAST_CHECK_EQ( boxi.point( index( 0u, 1u ) ), 	Point{ 0., 4. } );
			DOCTEST_FAST_CHECK_EQ( boxi.point( index( 3u, 1u ) ), 	Point{ 6., 4. } );
		...
		{	// 3d Box_indexer
			...
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 2.0, 0. } ), 	0u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 2.0, 0. } ), 	3u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 6.0, 0. } ), 	4u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 6.0, 0. } ), 	7u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 2.0, 4. } ), 	0u + 8u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 2.0, 4. } ), 	3u + 8u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 0., 6.0, 4. } ), 	4u + 8u );
			DOCTEST_FAST_CHECK_EQ( boxi.scalar_index( Point{ 8., 6.0, 4. } ), 	7u + 8u );
		}
	}
